"""
DIV2K Dataset Image Denoising Experiment
...
class ImageDenoiser:
    ...
    def compute_gradient(self, x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
...
def load_image(image_path: str, max_size: Optional[int] = None) -> np.ndarray:
    ...
    Args:
        ...
        max_size: Maximum size for resizing (optional, for large images)
    ...
    # Resize if image is too large (DIV2K images can be very large)
...
def run_experiments(dataset_path: str, noise_levels: List[float],
                   num_images: int = 5, max_image_size: Optional[int] = 512) -> Dict:
    """
    ...
    Args:
        ...
        max_image_size: Maximum image size for resizing (None to use original size)
        
    Returns:
        Experiment results dictionary
    """
    # Get test image list (use DIV2K_valid_HR as test set)
    test_dir = os.path.join(dataset_path, 'DIV2K_valid_HR')
    ...
    print("=" * 60)
    print("Starting DIV2K Image Denoising Experiment")
    print("=" * 60)
    ...
    for img_idx, img_file in enumerate(image_files):
        ...
        ground_truth = load_image(img_path, max_size=max_image_size)
        print(f"  Image size: {ground_truth.shape}")
...
def plot_visual_comparison(results: Dict, sigma: float = 25, 
    ...
    # Show zoomed region
    zoom_gt = ground_truth[start_h:end_h, start_w:end_w]
    zoom_noisy = noisy[start_h:end_h, start_w:end_w]
    zoom_ista = ista_result[start_h:end_h, start_w:end_w]
    zoom_fista = fista_result[start_h:end_h, start_w:end_w]
    zoom_admm = admm_result[start_h:end_h, start_w:end_w]
    
    # Create a composite zoomed view
    zoom_composite = np.hstack([zoom_gt, zoom_noisy, zoom_ista, zoom_fista, zoom_admm])
    axes[1, 2].imshow(zoom_composite, cmap='gray')
    axes[1, 2].set_title('Zoomed Region Comparison', fontsize=12, fontweight='bold')
    axes[1, 2].axis('off')
...
def main() -> None:
    ...
    print(f"  Dataset: DIV2K (DIV2K_valid_HR)")
    ...
    print(f"  Max Image Size: {max_image_size} (images will be resized if larger)")
    
    # Run experiments
    results = run_experiments(dataset_path, noise_levels, num_images, max_image_size)
